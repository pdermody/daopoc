...
describe("Governor Flow", async () => {
  ...
  beforeEach(async () => {
    ...
    console.log((await box.getSize()).toString())
    console.log((await box.getColor()).toString())
  })

  it("can only be changed through governance", async () => {
    await expect(box.setSize(55)).to.be.revertedWith("Ownable: caller is not the owner")
    await expect(box.setColor("red")).to.be.revertedWith("Ownable: caller is not the owner")
  })
